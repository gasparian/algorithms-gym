...
class Paginator {
  ...
  public:
    Paginator(Iterator f, Iterator l, size_t ps) {
      size_t curr_size = l - f;
      this->pages_no = curr_size / ps;
      if ( curr_size % ps ) {
        this->pages_no += 1;
      }
      size_t i = 0;
      while ( i < this->pages_no ) {
        this->pages.push_back(IteratorRange{
            next(f, ps * i), 
            next(f, min(ps * (i+1), curr_size))
          });
        i++;
      }
    }
...
//   Paginator full(v.begin(), v.end(), 3);

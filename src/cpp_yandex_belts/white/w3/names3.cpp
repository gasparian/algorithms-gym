...
string getClosest(const map<int, string>& names, const int& yearIn) {
    string res;
...
string getClosestSet(const map<int, string>& names, const int& yearIn, const string& orig) {
    vector<string> res;
    string ignore = orig;
    string out;
    for ( auto it=names.rbegin(); it!=names.rend(); it++ ) {
...
public:
  Person(string nameIn, string surnameIn, int year) {
      name[year] = nameIn;
      surname[year] = surnameIn;
  }
  ...
  string GetFullName(int year) const {
    ...
    if ( nameFlag ) {
        nameTmp = name.find(year)->second; 
    } else {
        nameTmp = getClosest(name, year);
    } 

    if ( surnameFlag ) {
        surnameTmp = surname.find(year)->second;
    } else {
...
//   const Person& person_ref = person;
